...
public UserDataInfo.User LoadUserData(){
		...
		return JsonUtility.FromJson<UserDataInfo.User> (dataStr);//読み込んだJSONファイルをPlayerData型に変換して返す
	}
	...
	public void SaveUserData(UserDataInfo.User user)
	{
		//初めに保存先を計算する　Application.dataPathで今開いているUnityプロジェクトのAssetsフォルダ直下を指定して、後ろに保存名を書く
